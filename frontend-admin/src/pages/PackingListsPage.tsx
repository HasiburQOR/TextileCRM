                  ...
                  <tr key={row.tempId} className="border-b border-slate-100 last:border-0">
                    ...
                    <td className="p-1 text-center">
                      {(() => {
                        ...
                        const coverImage = pickProductImage(product.images, row.colorName) ?? product.images[0]
                        const coverIndex = product.images.indexOf(coverImage)
                        return (
                          <button
                            type="button"
                            title="View photos"
...
function PackingListCartonPhoto({ carton, onOpen }: { carton: PackingCarton; onOpen: (images: LightboxImage[]) => void }) {
  ...
  const coverImage = pickProductImage(images, carton.colorName) ?? images[0]
  // This dialog's lightbox always opens at index 0 (no index param), so put
  // the color-matched image first rather than whatever order the API returned.
  const ordered = [coverImage, ...images.filter((img) => img !== coverImage)]
  return (
    <button
      type="button"
      title="View photos"
      onClick={() => onOpen(ordered.map((img) => ({ src: resolveMediaUrl(img.image), label: img.customLabelName || img.label })))}
      className="h-9 w-9 overflow-hidden rounded border border-slate-200 hover:ring-2 hover:ring-slate-300"
    >
      <img src={resolveMediaUrl(coverImage.image)} alt="" className="h-full w-full object-cover" />
    </button>
