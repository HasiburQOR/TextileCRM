  ...
  const createMutation = useMutation({
    mutationFn: async (payload: ProductCreateInput) => {
      ...
      if (packingListFile) {
        ...
        form.append("factoryPackingList", packingListFile)
        await api.post(`/products/${product.id}/upload_factory_packing_list/`, form, {
  ...
  return (
    <Dialog open onClose={onClose} title="New Sourcing Request" className="max-w-[95vw] xl:max-w-7xl">
      <form onSubmit={handleSubmit} className="flex flex-col gap-4">
        ...
        <div className="flex flex-col gap-1.5">
          <label className="text-xs font-medium text-slate-600">Factory Packing List (photo/scan)</label>
          <div className="flex items-center gap-2">
            <Button type="button" variant="outline" size="sm" onClick={() => packingListInputRef.current?.click()}>
              <FileImage className="h-3.5 w-3.5" />
              {packingListFile ? "Change File" : "Choose File"}
            </Button>
            {packingListFile && <span className="truncate text-xs text-slate-500">{packingListFile.name}</span>}
            <input
              ref={packingListInputRef}
              type="file"
              accept="image/*,.pdf"
              className="hidden"
...
function VariantRowsEditor({ variants, columns, onChange, onAddColumn, onAddRow }: {
  ...
  return (
    <div className="flex flex-col gap-2">
            ...
            {variants.map((row, i) => {
              ...
              return (
                <tr key={row.tempId} className="border-b border-slate-100 last:border-0">
                  ...
                  <td className="p-1">
                    <Input className="h-7 w-14 text-xs" type="number" min={1} value={row.innerBundle} onChange={(e) => updateVariant(i, { innerBundle: Number(e.target.value) })} />
                  </td>
                  <td className="p-1">
                    <Input className="h-7 w-14 text-xs" type="number" min={1} value={row.cartonNoFrom ?? ""} onChange={(e) => updateVariant(i, { cartonNoFrom: e.target.value ? Number(e.target.value) : null })} />
                  </td>
                  <td className="p-1">
                    <Input className="h-7 w-14 text-xs" type="number" min={1} value={row.cartonNoTo ?? ""} onChange={(e) => updateVariant(i, { cartonNoTo: e.target.value ? Number(e.target.value) : null })} />
                  </td>
                  ...
                  <td className="p-1">
                    <Input className="h-7 w-14 text-xs" type="number" step="0.01" min={0} max={999999} value={row.netWeight ?? ""} onChange={(e) => updateVariant(i, { netWeight: e.target.value ? Number(e.target.value) : null })} />
                  </td>
                  ...
                  <td className="p-1">
                    <Input className="h-7 w-12 text-xs" type="number" step="0.01" min={0} max={999999} value={row.ctnLength ?? ""} onChange={(e) => updateVariant(i, { ctnLength: e.target.value ? Number(e.target.value) : null })} />
                  </td>
                  <td className="p-1">
                    <Input className="h-7 w-12 text-xs" type="number" step="0.01" min={0} max={999999} value={row.ctnWidth ?? ""} onChange={(e) => updateVariant(i, { ctnWidth: e.target.value ? Number(e.target.value) : null })} />
                  </td>
                  <td className="p-1">
                    <Input className="h-7 w-12 text-xs" type="number" step="0.01" min={0} max={999999} value={row.ctnHeight ?? ""} onChange={(e) => updateVariant(i, { ctnHeight: e.target.value ? Number(e.target.value) : null })} />
                  </td>
      ...
      <div className="flex flex-wrap gap-4 rounded-md border border-slate-200 bg-slate-50 px-3 py-2 text-xs text-slate-600">
