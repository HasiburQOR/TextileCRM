...
**Identifier note:** `style_no` and `pattern_no` on every row here are the *same values* generated once at Sourcing Intake (see Module Documentation Pack, Module 4, "Identifier Glossary") — this module displays and finalizes them, it never generates or re-enters them. `po_no` is likewise the same manually-entered PO Reference from the Sister Profile, not a system-generated code. Do not confuse any of these with the Buyer Reference Code (`BUY-0000`) or Sister Profile Reference Code (`SIS-0000`), which are separate internal tracking IDs and do not appear on the packing list document itself.
